...
`npx @open-slide/cli init` generates an `AGENTS.md` at the root with the
framework's hard rules, plus a `skills/` folder with five
...
1. **Read the rules.** `AGENTS.md` at the workspace root
...
convention that Copilot, Codex, Cursor, Gemini CLI, OpenCode, Windsurf, and
Zed already look for. The workspace keeps one rules file and one canonical
skills directory: `AGENTS.md` plus `.agents/skills/`.
